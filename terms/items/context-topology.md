---
slug: context-topology
name: Context Topology
category: Context
title: Context Topology
aliases:
short_description: Context Topology is the map of where context lives and how it moves
termStatus: Emerging practitioner shorthand
researchBasis: Lewis et al., Retrieval-Augmented Generation
sources:
- https://arxiv.org/abs/2005.11401
---

## Term status

Emerging practitioner shorthand.

## Meaning

Context Topology concerns the selection, quality, lineage or delivery of information presented to an agent at runtime.

## Boundary

It is not a defined product category. Make the retrieval, freshness, ownership, permissions and token-budget rules explicit.

## How it is used

Context Topology is used to describe where context lives and how it travels between users, agents, stores and tools. It should make cross-tenant boundaries, caching and replication visible.

## Evidence

[Lewis et al., Retrieval-Augmented Generation](https://arxiv.org/abs/2005.11401) provides the relevant primary source or established reference. For coined labels, it is background for the underlying concept—not evidence that the label itself is standard.
